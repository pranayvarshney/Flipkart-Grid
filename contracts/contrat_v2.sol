...
contract Product is ERC721, ERC721Enumerable, ERC721URIStorage, ERC721Burnable, Ownable{
    ...
    function checkValidity(string memory serialNum) external view returns(bool){
    ...
    modifier sufficientSuperCoinBalance(uint superCoinNum){
        // will check if customer has sufficient superCoin balance or not
        require(superCoinContract.balanceOf(msg.sender) >= superCoinNum);
        _;
    }
    ...
    function extendWarranty(string memory serialNum, uint newWarrantyPeriod, uint superCoinNum) external payable sufficientSuperCoinBalance(superCoinNum){
