...
contract Product is ERC721, ERC721Enumerable, ERC721URIStorage, ERC721Burnable, Ownable, KeeperCompatibleInterface {
    ...
    // uint _interval;
    // uint lastTimeStamp;
    constructor(uint interval) ERC721("Product", "PDCT") {
        // _interval = interval;
    ...
    //supercoin interface
    address scAddress = 0x0f5EAc0820153C7609052a0f10491995c31Bc9e7;
    SuperCoinInterface superCoinContract = SuperCoinInterface(scAddress);
 
    //Chain link keeper to check the validity of warranty
 
    string[] invalidSerialNum;
    function checkUpkeep(bytes calldata /* checkData */) external view override returns (bool upkeepNeeded, bytes memory /* performData */) {
        // upkeepNeeded = (block.timestamp - lastTimeStamp) > _interval;
    ...
    modifier isFlipkart(){
        require(msg.sender == 0x5B38Da6a701c568545dCfcB03FcB875f56beddC4, "Only flipkart can make this change");
        _;
    }
    ...
    function awardBenefit(address to, string memory serialNum) internal{
        // uint numOfPurchases = balanceOf(to);
        emit benefits(to,2);
        superCoinContract.raise(to, serialNum, 2);
        superCoinContract.blockSerialNumForSC(serialNum);
        superCoinContract.safeMint(to);
        // if(numOfPurchases%3 == 0){
        //     emit benefits(to, 5);    
        //     awarding 5 supercoins to first time buyers
        //     todo: send coins to the customer
        // }
    } 
    ...
    function extendWarranty(string memory serialNum, uint newWarrantyPeriod, uint superCoinNum, uint price) external payable sufficientSuperCoinBalance(superCoinNum, msg.sender){
        require(msg.value >= (price - superCoinNum), "Insufficient Balance");
        payable(msg.sender).transfer(msg.value - (price - superCoinNum));
